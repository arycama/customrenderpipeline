...
RayCone ComputeRayConeFromGBuffer(SurfaceHit gbuffer)
{
	...
	rc.spreadAngle = _RaytracingPixelSpreadAngle;
...
float ComputeTextureLOD(float2 size, Ray ray, SurfaceHit surf, RayCone cone, float scale = 1.0)
{
	// Eq 34
...
float ComputeTextureLOD(Texture2D<float4> targetTexture, Ray ray, SurfaceHit surf, RayCone cone, float scale)
{
	float2 size;
	targetTexture.GetDimensions(size.x, size.y);
	return ComputeTextureLOD(size, ray, surf, cone);
}

float ComputeTextureLOD(Texture2D<float2> targetTexture, Ray ray, SurfaceHit surf, RayCone cone, float scale)
{
	float2 size;
	targetTexture.GetDimensions(size.x, size.y);
	return ComputeTextureLOD(size, ray, surf, cone);
...
float ComputeTextureLOD(Texture2DArray<float4> targetTexture, Ray ray, SurfaceHit surf, RayCone cone, float scale)
{
	float3 size;
	targetTexture.GetDimensions(size.x, size.y, size.z);
	return ComputeTextureLOD(size.xy, ray, surf, cone);
}
