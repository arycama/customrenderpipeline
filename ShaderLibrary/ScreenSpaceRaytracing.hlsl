...
float3 ScreenSpaceRaytrace(float3 rayOrigin, float3 rayDirection, uint maxSteps, float thickness, Texture2D<float> depth, uint maxMip, out bool validHit)
{
	...
	float t = Min2(abs(0.5 / rayDirection.xy));
	...
	{
		rayPos.xyz = rayOrigin + t * rayDirection;
       ...
       // int2 mipOffset = _DepthPyramidMipLevelOffsets[mipLevel];
		...
		float4 dist = bounds / rayDirection.xyzz - (rayOrigin.xyzz / rayDirection.xyzz);
