...
/// class specific
#define MaxThreadGroupsToRun    800
...
void ParallelSortCountUInt(uint localID, uint groupID, uint ShiftBit)
{
    ...
    int BlockSize = ElementsPerThread * ThreadGroupSize;
    ...
    for (uint BlockCount = 0; BlockCount < NumBlocksToProcess; BlockCount++, BlockIndex += BlockSize)
    {
        ...
        uint srcKeys[ElementsPerThread];
		srcKeys[0] = (BlockIndex < NumKeys ? SourceKeys[BlockIndex] : 0xffffffff);
		srcKeys[1] = (BlockIndex + ThreadGroupSize < NumKeys ? SourceKeys[BlockIndex + ThreadGroupSize] : 0xffffffff);
		srcKeys[2] = (BlockIndex + ThreadGroupSize * 2 < NumKeys ? SourceKeys[BlockIndex + ThreadGroupSize * 2] : 0xffffffff);
		srcKeys[3] = (BlockIndex + ThreadGroupSize * 3 < NumKeys ? SourceKeys[BlockIndex + ThreadGroupSize * 3] : 0xffffffff);
...
void ParallelSortReduceCount(uint localID, uint groupID)
{
    ...
    uint BaseIndex = (groupID % NumReduceThreadgroupPerBin) * ElementsPerThread * ThreadGroupSize;

    // Calculate partial sums for entries this thread reads in
    uint threadgroupSum = 0;
    for (uint i = 0; i < ElementsPerThread; ++i)
    {
        uint DataIndex = BaseIndex + (i * ThreadGroupSize) + localID;
        threadgroupSum += (DataIndex < NumThreadGroups) ? SumTable[BinOffset + DataIndex] : 0;
    }
...
groupshared uint LDS[ElementsPerThread][ThreadGroupSize];
void ParallelSortScanPrefix(uint numValuesToScan, uint localID, uint groupID, uint BinOffset, uint BaseIndex, bool AddPartialSums)
{
    ...
    for (uint i = 0; i < ElementsPerThread; i++)
    {
        ...
        uint col = ((i * ThreadGroupSize) + localID) / ElementsPerThread;
        uint row = ((i * ThreadGroupSize) + localID) % ElementsPerThread;
		LDS[row][col] = (DataIndex < numValuesToScan) ? ScanSource[BinOffset + DataIndex] : 0;
	}
    ...
    for (uint i = 0; i < ElementsPerThread; i++)
    {
        uint tmp = LDS[i][localID];
        LDS[i][localID] = threadgroupSum;
        threadgroupSum += tmp;
    }
    ...
    for (uint i = 0; i < ElementsPerThread; i++)
    {
        ...
        if (DataIndex < numValuesToScan)
			ScanDest[BinOffset + DataIndex] = LDS[row][col] + partialSum;
	}
}
...
void ParallelSortScatterUInt(uint localID, uint groupID, uint ShiftBit)
{
    ...
    if (localID < SortBinCount)
        BinOffsetCache[localID] = SumTable[localID * NumThreadGroups] + groupID;
    ...
    for (int BlockCount = 0; BlockCount < NumBlocksToProcess; BlockCount++, BlockIndex += BlockSize)
    {
        ...
        // Pre-load the key values in order to hide some of the read latency
        uint srcKeys[ElementsPerThread];
        srcKeys[0] = (DataIndex < NumKeys ? SourceKeys[DataIndex] : 0xffffffff);
		srcKeys[1] = (DataIndex + ThreadGroupSize < NumKeys ? SourceKeys[DataIndex + ThreadGroupSize] : 0xffffffff);
		srcKeys[2] = (DataIndex + ThreadGroupSize * 2 < NumKeys ? SourceKeys[DataIndex + ThreadGroupSize * 2] : 0xffffffff);
		srcKeys[3] = (DataIndex + ThreadGroupSize * 3 < NumKeys ? SourceKeys[DataIndex + ThreadGroupSize * 3] : 0xffffffff);

        uint srcValues[ElementsPerThread];
        srcValues[0] = (DataIndex < NumKeys ? SourcePayload[DataIndex] : 0);
		srcValues[1] = (DataIndex + ThreadGroupSize < NumKeys ? SourcePayload[DataIndex + ThreadGroupSize] : 0);
		srcValues[2] = (DataIndex + ThreadGroupSize * 2 < NumKeys ? SourcePayload[DataIndex + ThreadGroupSize * 2] : 0);
		srcValues[3] = (DataIndex + ThreadGroupSize * 3 < NumKeys ? SourcePayload[DataIndex + ThreadGroupSize * 3] : 0);
        ...
        for (int i = 0; i < ElementsPerThread; i++)
        {
            ...
            {
                DestKey[totalOffset] = localKey;
                DestPayload[totalOffset] = localValue;
            }
            ...
            DataIndex += ThreadGroupSize;	// Increase the data offset by thread group size
        }
    }
}