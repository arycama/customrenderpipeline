...
struct CurveParamsUser
{
	...
	CurveParamsUser Create()
	{
		CurveParamsUser output;
		output.m_toeStrength = 0.0f;
		output.m_toeLength = 0.5f;
		output.m_shoulderStrength = 0.0f; // white point
		output.m_shoulderLength = 0.5f;

		output.m_shoulderAngle = 0.0f;
		output.m_gamma = 1.0f;
		return output;
	}
};
...
struct CurveParamsDirect
{
	...
	CurveParamsDirect Create()
	{
		CurveParamsDirect output = 0;
		output.Reset();
		return output;
	}
};
...
void CreateCurve(FullCurve dstCurve, CurveParamsDirect srcParams)
{
...
void CalcDirectParamsFromUser(CurveParamsDirect dstParams, CurveParamsUser srcParams)
{
	dstParams = CurveParamsDirect::Create();
