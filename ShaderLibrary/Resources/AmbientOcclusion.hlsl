...
float4 FragmentCompute(VertexFullscreenTriangleOutput input) : SV_Target
{
	...
	float3 normalV = mul((float3x3) WorldToView, GBufferNormal(GBufferNormalRoughness[input.position.xy], V));
...
float4 FragmentCombine(VertexFullscreenTriangleOutput input) : SV_Target
{
	...
	bentNormalOcclusion.xyz = GBufferNormal(bentNormalOcclusion, V);
