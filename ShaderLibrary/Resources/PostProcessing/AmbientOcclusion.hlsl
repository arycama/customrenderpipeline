...
{
	return MultiplyPoint3x4(unity_MatrixV, PixelToWorld(float3(screenPos, _CameraDepth[screenPos]))) * float3(1, 1, -1);
}
...
float3 Fragment(float4 position : SV_Position) : SV_Target
{
	...
	float3 normalV = mul((float3x3) unity_MatrixV, worldNormal) * float3(1, 1, -1);
	
	float3 cPosV = ComputeViewspacePosition(position.xy);
	float3 viewV = normalize(-cPosV);
	
	//float2 noise = InterleavedGradientNoise(position.xy, 0); //	_BlueNoise2D[position.xy % 128];
	...
	for (float i = 0; i < _DirectionCount; i++)
	{
		...
		for (uint side = 0; side < 2; side++)
		{
			...
			for (float j = noise.y; j < _SampleCount; j++)
			{
				...
				if (all(floor(sTexCoord) == floor(position.xy)))
					continue;
