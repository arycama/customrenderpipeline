...
FragmentOutput Fragment(float4 position : SV_Position, float2 uv : TEXCOORD0, float3 worldDir : TEXCOORD1)
{
	...
	output.normalRoughness = float4(PackGBufferNormal(normal, V), roughness, 0);
	output.bentNormalOcclusion = float4(PackGBufferNormal(bentNormal, V), bentNormalOcclusion.b, 0);
	return output;
}