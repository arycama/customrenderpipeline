...
float3 SampleLuminance(float3 rayDirection, float xi, uint colorIndex, bool rayIntersectsGround, float maxRayLength, float3 maxLuminance)
{
	...
	float3 rgbPdf = (lightTransmittance + multiScatter) * (scatter.xyz + scatter.w) * viewTransmittance / maxLuminance;
	float pdf = dot(rgbPdf, rcp(3.0));
...
FragmentOutputTemporal FragmentTemporal(VertexFullscreenTriangleMinimalOutput input)
{
	...
	if (!IsFirst && all(saturate(previousUv) == previousUv))
	{
		...
		if (historyWeight)
		{
			...
			history *= PreviousToCurrentExposure;
