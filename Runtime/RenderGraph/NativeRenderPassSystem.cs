...
public class NativeRenderPassSystem
{
    ...
    private NativeRenderPassData currentPassData = new(), previousPassData = new();

    public void SetSize(Int3 size)
    {
        currentPassData.size = size;
    }

    public void SetSubPassFlags(SubPassFlags flags)
    {
        currentPassData.flags = flags;
    }
    ...
    {
        currentPassData.depthAttachment = new AttachmentDescriptor(format) { loadAction = loadAction, storeAction = storeAction, loadStoreTarget = loadStoreTarget };
        currentPassData.hasDepthAttachment = true;
    }
    ...
    {
        currentPassData.colorAttachments.Add(new AttachmentDescriptor(format) { loadAction = loadAction, storeAction = storeAction, loadStoreTarget = loadStoreTarget, clearColor = clearColor });
    }

    public void BeginRenderPass(CommandBuffer command)
    {
        ...
        // Reset
        currentPassData.size = 0;
        currentPassData.colorAttachments.Clear();
        currentPassData.depthAttachment = default;
        currentPassData.hasDepthAttachment = false;
    }
