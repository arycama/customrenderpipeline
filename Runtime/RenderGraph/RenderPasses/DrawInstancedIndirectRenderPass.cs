...
public class DrawInstancedIndirectRenderPass<T> : DrawRenderPass<T>
{
	...
	public void Initialize(Mesh mesh, int submeshIndex, Material material, ResourceHandle<GraphicsBuffer> indirectArgsBuffer, Int2 size, int viewCount, int passIndex = 0, float depthBias = 0.0f, float slopeDepthBias = 0.0f, bool zClip = true, int argsOffset = 0, bool isScreenPass = false)
	{
		this.mesh = mesh;
