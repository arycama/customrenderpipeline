...
public class VolumetricCloudShadow : ViewRenderFeature
{
	...
	public override void Render(in ReadOnlySpan<ViewParameter> viewParameters, in ViewPassData viewPassData, in DisplayData displayOutputData, ScriptableRenderContext context)
    {
        ...
        var cameraPosition = viewPassData.position;
        ...
        var worldToLight = lightRotation.Inverse;
        ...
        for (var z = 0; z < 2; z++)
        {
            for (var y = 0; y < 2; y++)
            {
                for (var x = 0; x < 2; x++)
                {
                    ...
                    var localPoint = worldToLight.Rotate(worldPoint);
                    ...
                    if (Geometry.IntersectRaySphere(worldPoint - planetCenter, lightDirection, planetRadius + settings.StartHeight + settings.LayerThickness, out var hits) && hits.y > 0.0f)
                    {
                        var worldPoint1 = worldPoint + lightDirection * hits.y;
                        var localPoint1 = worldToLight.Rotate(worldPoint1);
        ...
        var isFirstShadow = !perCameraShadow.TryGetValue(viewPassData.viewId, out var cloudShadow);
        if (isFirstShadow)
        {
            ...
            perCameraShadow.Add(viewPassData.viewId, cloudShadow);
        }
        ...
        // Cloud coverage
		var isFirst = !perCameraCoverage.TryGetValue(viewPassData.viewId, out var previousCloudCoverage);
		if (isFirst)
		{
			...
			perCameraCoverage.Add(viewPassData.viewId, previousCloudCoverage);
		}
