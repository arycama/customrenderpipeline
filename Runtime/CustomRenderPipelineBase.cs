...
public abstract class CustomRenderPipelineBase : RenderPipeline
{
    ...
    protected virtual void CollectViewRenderData(List<Camera> cameras, ScriptableRenderContext context, List<ViewRenderData> viewRenderDatas)
    {
        foreach (var camera in cameras)
        {
            ...
            viewRenderDatas.Add(new ViewRenderData(camera.ViewSize(), camera.nearClipPlane, camera.farClipPlane, camera.TanHalfFov(), camera.transform.WorldRigidTransform(), camera, context, cullingParameters, target, VRTextureUsage.None, SinglePassStereoMode.None, 1, string.Empty, GraphicsFormat.R8G8B8A8_SRGB, screenSize, 1, false));
